function [lon_ggm, lat_ggm, height_anomaly_ggm, gravity_anomaly_ggm] = GGM_Data (array)
  ...
  %INPUT - 
  %1. H (string) - Vector containing path to all the files containing GGM data 
  %OUTPUT - 
  ...
  while i <= rows(array)
    fid = fopen (array(i,:), "r");
    ggm = textscan (fid,'%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f%f', 'Delimiter',' ','MultipleDelimsAsOne',1, 'HeaderLines',56);
    fclose(fid);
